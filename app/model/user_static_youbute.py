...
class UserStaticYoutube(Base):
    ...
    channel_name = Column(String(400), nullable=False)
    sub_date = Column(DATE, nullable=True)

    def __repr__(self):
        return "<UserStaticYoutube(user_id='%s', channel_id,='%s' member_name='%s',channel_name='%s', sub_date='%s')>" % (
            self.user_id,
            self.channel_id,
            self.member_name,
            self.channel_name,
    ...
    FIELDS = {"user_id": str, "channel_id": str, "member_name": str, "channel_name": str,  "sub_date": DATE}
